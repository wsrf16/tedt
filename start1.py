...
# 构建“蚁群算法”的主体
class ACO(object):
    def __init__(self, ant_num=50, max_iter=150, alpha=1, beta=5, rho=0.1, Q=1):
        self.ants_num = ant_num       # 蚂蚁个数
        self.max_iter = max_iter      # 蚁群最大迭代次数
        ...
        ###########################
        self.ants_path_seed = np.zeros(self.ants_num).astype(int)  # 记录一次迭代过程中每个蚂蚁的初始城市下标
        self.ants_path = np.zeros((self.max_iter, self.ants_num))  # 记录各迭代所有蚂蚁的路径长度信息
        self.ants_best_path = np.zeros(self.max_iter)              # 记录各迭代整个蚁群的“历史”最短路径长度
        ###########################
    ...
    def deal_data(self, filename):
        ...
        self.tau_mat = np.ones((self.cities_num, self.cities_num))                       # 4. 初始化信息素矩阵
    ...
    def solve(self):
        ...
        while iter_num < self.max_iter:
            ...
            for k in range(self.ants_num):
                city_index1 = self.ants_path_seed[k]                                # 每只蚂蚁访问的第一个城市下标
                ...
                for i in range(self.cities_num-1):                             # 对余下的城市进行访问
                    up_proba = np.zeros(self.cities_num-len(tabu))             # 初始化状态迁移概率的分子
                    for j in range(self.cities_num-len(tabu)):
                        up_proba[j] = np.power(self.tau_mat[city_index1][non_tabu[j]], self.alpha) * \
                                      np.power(self.eta_mat[city_index1][non_tabu[j]], self.beta)
                    p = up_proba/sum(up_proba)                             # 每条可能子路径上的状态迁移概率
                    while True:                                                # 提取出下一个城市的下标
                        random_num = np.random.rand()
                        index_need = np.where(p > random_num)[0]
                        if len(index_need) > 0:
                ...
                self.ants_path[iter_num][k] = Ant(ant_path.path).length
                if iter_num == 0 and k == 0:                                    # 完成对最佳路径城市的记录
                    self.best_cities = ant_path.path
                else:
                    if self.ants_path[iter_num][k] < Ant(self.best_cities).length: self.best_cities = ant_path.path
                tabu.append(tabu[0])                                           # 每次迭代完成后，使禁忌城市下标列表形成完整闭环
                for l in range(self.cities_num):
                    delta_phero_mat[tabu[l]][tabu[l+1]] += self.Q/self.ants_path[iter_num][k]

            self.ants_best_path[iter_num] = Ant(self.best_cities).length

            self.update_phero_mat(delta_phero_mat)                             # 更新信息素矩阵
            iter_num += 1

    def update_phero_mat(self, delta):
        self.tau_mat = (1 - self.rho) * self.tau_mat + delta
    ...
    def random_seed(self):                                                     # 产生随机的起始点下表，尽量保证所有蚂蚁的起始点不同
        if self.ants_num <= self.cities_num:                                   # 蚂蚁数 <= 城市数
            self.ants_path_seed[:] = np.random.permutation(range(self.cities_num))[:self.ants_num]
        else:                                                                  # 蚂蚁数 > 城市数
            self.ants_path_seed[:self.cities_num] = np.random.permutation(range(self.cities_num))
            temp_index = self.cities_num
            while temp_index + self.cities_num <= self.ants_num:
                self.ants_path_seed[temp_index:temp_index + self.cities_num] = np.random.permutation(range(self.cities_num))
                temp_index += self.cities_num
            temp_left = self.ants_num % self.cities_num
            if temp_left != 0:
                self.ants_path_seed[temp_index:] = np.random.permutation(range(self.cities_num))[:temp_left]

    def display(self):                                                         # 数据可视化展示
        plt.figure(figsize=(6, 10))
        plt.subplot(211)
        plt.plot(self.ants_path, 'g.')
        plt.plot(self.ants_best_path, 'r-', label='history_best')
